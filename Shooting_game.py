...
running = True
while running:
    clock.tick(1000)        #capped game to 1000fps, if making it a lower cap increase object speeds to retain same pace of game
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if game_active:
            if event.type == pygame.KEYDOWN:    #player controls
                if event.key == pygame.K_w:
                    ...
                    player_y_change = player_speed
                elif event.key == pygame.K_SPACE:   #spawn bullet object into game
    ...
    if game_active:
        ...
        #respawning shot enemies:
        for enemy in enemies:
            enemy.update()
            if enemy.active == False:

                if np.random.random() < 1:  # 100% chance of respawning every frame
